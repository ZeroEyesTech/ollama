...
for TARGETARCH in ${BUILD_ARCH}; do
    ...
    docker cp builder-$TARGETARCH:/go/src/github.com/ZeroEyesTech/ollama/ollama ./dist/ollama-linux-$TARGETARCH

    if [ "$TARGETARCH" = "amd64" ]; then
        docker cp builder-$TARGETARCH:/go/src/github.com/ZeroEyesTech/ollama/dist/deps/ ./dist/
    fi
